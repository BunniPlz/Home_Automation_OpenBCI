...
class DataProcessing_User {
  ...
  public void process(float[][] data_newest_uV, //holds raw bio data that is new since the last call
    ...
    float EEG_value_uV;
    if(w_p300speller.countdownCurrent == 0) { //If countdownCurrent is equal to 0, then we will begin data collection.
      println("Begin multichannel process");
  ...
  public void processMultiChannel(float[][] data_newest_uV, float[][]data_long_uV, float[][] data_forDisplay_uV, FFT[] fftData) {
    ...
    if (sample_position == 250) { //Once 250 samples are taken, recollect starting at sample_position # 0. #trials will increase. This continues until trial_count reaches TOTAL TRIAL COUNT, at which point it will perform post-processing.
   ...
   void CheckSNR() {
     //Variables are displayed below for reference.
      ...
      for(int t = 50; t < 200; t++) { //Checking samples of interest for the P300.
  ...
  //Finds Peak Frequency, among some other information. Taken from EEG Processing.
   void findPeakFrequency(FFT[] fftData, int Ichan) {
    ...
    rms[sample_position][Ichan][previous_rand_index] = rms[sample_position][Ichan][previous_rand_index] + detectedPeak[Ichan].rms_uV_perBin; //Multiply by 10000 to avoid truncation.
    println("Raw RMS :" + detectedPeak[Ichan].rms_uV_perBin);
    println("Rms of signal " + "sample position " + sample_position + " is: " + rms[sample_position][Ichan][previous_rand_index]);
    ...
    background_rms[sample_position][Ichan][previous_rand_index] = background_rms[sample_position][Ichan][previous_rand_index] + sqrt(sum_pow/count);
    println("Background rms of " + "sample position " + sample_position + " is: " + background_rms[sample_position][Ichan][previous_rand_index] + " for rand_index: " + previous_rand_index);
